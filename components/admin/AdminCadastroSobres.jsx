...
export default function AdminCadastroSobres(props) {
    ...
    async function handleSubmit(e) {
        ...
        if (props.id) {
            response = await axios.put(URL_API + '/abouts/' + props.id, data)
            
        } else {
            //faz o envio dos dados captados no formulário de cadastro de noticias
            response = await axios.post(URL_API + '/abouts', data)
    ...
    //         }
    //     }
    //     const resp = await axios.post(URL_API + '/abouts/' + response.data.id + '/images', formData, config)
    // }
    setTimeout( () => {
        Router.push('/admin/listagemSobre')
    },1000) 
    ...
    useEffect(
        () => {
            async function loadingAbout() {
                ...
                const response = await axios.get(URL_API + '/abouts/' + props.id)
                    ...
                    <div className="p-1 mx-5">
                        <div className="columns is-desktop">
                            <div className="column">
                                ...
                                <h2 className="is-size-4">Cadastar uma nova Sobre</h2>
                            </div>
                        </div>
                        {/* {JSON.stringify(data)} */}
                        <Menssagem close={ closeMsg } visible={ msgVisible }>Sobre Cadastrada com sucesso !</Menssagem>
                        <div className="columns is-desktop">
                            <div className="column">
                                <div className="field">
                                    <div className="control has-icons-left has-icons-right">
                                        <input onChange={handleChange} value={data.category} onChange={handleChange} value={data.category} required className="input is-medium" type="text" name="category" placeholder="Categoría" />
                                ...
                                <div className="field">
                                    <div className="control has-icons-left has-icons-right">
                                        <textarea onChange={handleChange} value={data.description} required className="textarea pt-5" type="text" name="description" placeholder="Conteúdo da matéria"></textarea>
                                        <span className="icon is-left">
