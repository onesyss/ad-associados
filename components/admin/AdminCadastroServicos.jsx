...
export default function AdminCadastroServicos(props) {
    ...
    async function handleSubmit(e) {
        ...
        if (props.id) {
            response = await axios.put(URL_API + '/services/' + props.id, data)
            
        } else {
            //faz o envio dos dados captados no formulário de cadastro de noticias
            response = await axios.post(URL_API + '/services', data)
    ...
    //         }
    //     }
    //     const resp = await axios.post(URL_API + '/services/' + response.data.id + '/images', formData, config)
    // }
    setTimeout( () => {
        Router.push('/admin/listagemServico')
    },1000) 
    ...
    useEffect(
        () => {
            async function loadingService() {
                ...
                const response = await axios.get(URL_API + '/services/' + props.id)
                    ...
                    <div className="p-1 mx-5">
                        <div className="columns is-desktop">
                            <div className="column">
                                ...
                                <h2 className="is-size-4">Cadastar uma nova Serviços</h2>
                            </div>
                        </div>
                        {/* {JSON.stringify(data)} */}
                        <Menssagem close={ closeMsg } visible={ msgVisible }>Serviço Cadastrada com sucesso !</Menssagem>
                        <div className="columns is-desktop">
                            <div className="column">
                                <div className="field">
                                    <div className="control has-icons-left has-icons-right">
                                        <input onChange={handleChange} value={data.category} required onChange={handleChange} value={data.category} className="input is-medium" type="text" name="category" placeholder="Categoría" />
